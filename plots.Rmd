...
```{r}
gRootName = "yoglen"
outputDir = "/home/glen/yoglen" #set home directory here
gSaveDefault = F
...
if(addHist)
{
  ...
  save=gSaveDefault
  load=T
  file = sprintf("%s/data/menopause_ages.csv",outputDir)
  if (file.exists(file) & load)
  {
    menoAges = read.csv(file)[,"menopause_age"]
  } else
  {
    menoAges = numeric()
    for (i in 1:length(Xmi)) menoAges = c(menoAges,Xmi[[i]][,"age_menopause"])
    if(save) write.csv(data.frame(menopause_age=menoAges),file,row.names=F)
  }


  d = density(menoAges,bw=1.5)
...
#Outcome summary
```{r}
temp = read.csv(sprintf("%s/data/nhanes_clinical_variable_summary.csv",outputDir))
